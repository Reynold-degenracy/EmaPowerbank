...
`feedback/` 保存每次提交后的反馈包目录。每个目录命名为 `feedback-<id>-<timestamp>/`，包含 `feedback.json` 和可选上传图片。JSON 文件会记录反馈 id、时间戳、提交用户、文字描述、附件元数据、审核状态和奖励元数据。管理员可以审核反馈、预览图片附件、通过并发放非零奖励，或拒绝且不发放余额。
